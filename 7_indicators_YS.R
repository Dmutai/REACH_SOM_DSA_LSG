...
data_indicators <- data %>% mutate (
  ...
  snfi_nc_index2 = case_when(
      ...
      str_count(nfi_items_available,pattern = " ") - 
        ifelse(sm_selected(nfi_items_available,any=c("none")),1,0) >=  5  ~ 1 ,
      
      str_count(nfi_items_available,pattern = " ") - 
        ifelse(sm_selected(nfi_items_available,any=c("none")),1,0)  >= 1  ~ 0 
  ...
  wash_index1 = case_when(
    sm_selected(water_treatment_methods,none=c("boiling", "cloth_filter", "other_filter", "aquatabs") ) &
      water_sources_primary %in% c("unprot_well", "berkad", "water_tank_tap", "water_trucking_distrib", "borehole_pump")  ~ 3,
    
    sm_selected(water_treatment_methods,any=c("boiling", "cloth_filter", "other_filter", "aquatabs") ) &
      water_sources_primary %in% c("unprot_well", "berkad", "water_tank_tap", "water_trucking_distrib", "borehole_pump")  ~ 2,
    
    water_sources_primary %in% c("water_kiosk", "vendors_shop", "piped", "prot_well_no_handpump", "prot_well_handpump")  ~ 1
  ...
  wash_nc_index6 = case_when(
    sm_selected(water_access_distance_min,exactly = "more_60") |
      water_sources_primary %in% c("unprot_well", "berkad", "water_tank_tap", "water_trucking_distrib", "borehole_pump")  ~ 1,
    
    sm_selected(water_access_distance_min,any = c("less_15","15_30","30_60") ) &
      water_sources_primary %in% c("water_kiosk", "vendors_shop", "piped", "prot_well_no_handpump", "prot_well_handpump")  ~ 0,
  ...
  nutrition_index1 = case_when(
    nutrition_prop_malnurish %in% c("all","many","some") ~ 1,
    nutrition_prop_malnurish %in% c("few","none") ~ 0,
  ),
  ...
  education_index1 = case_when(
    sm_selected(education_barriers_boys,any = c("child_recruited_ag", "displacement_conflict")) ~ 4,
    sm_selected(education_barriers_boys,any = c("security_concerns", "child_lack_documentation", "no_gender_separ", "help_at_home", "costs", "child_pycho_distress", "work_outside_home", "flood", "marriage_pregnant")) ~ 3,
    sm_selected(education_barriers_boys,any = c("far_away", "closed", "poor_infrastructure", "lack_quali_staff", "no_wash_at_school", "overcrowded") ) ~ 2,
    sm_selected(education_barriers_boys,any = c("no_problem", "no_aware_education_opportunities", "parents_no_value_edu", "parents_no_approve_curric", "cultural_beliefs", "language")) ~ 1,
  ),
  
  #### Education Critical index 2
  ###YS education should not have a score of 4. 
  education_index2 = case_when(
    sm_selected(education_barriers_girls,any = c("child_recruited_ag", "displacement_conflict")) ~ 4,
    sm_selected(education_barriers_girls,any = c("security_concerns", "child_lack_documentation", "no_gender_separ", "help_at_home", "costs", "child_pycho_distress", "work_outside_home", "flood", "marriage_pregnant")) ~ 3,
    sm_selected(education_barriers_girls,any = c("far_away", "closed", "poor_infrastructure", "lack_quali_staff", "no_wash_at_school", "overcrowded") ) ~ 2,
    sm_selected(education_barriers_girls,any = c("no_problem", "no_aware_education_opportunities", "parents_no_value_edu", "parents_no_approve_curric", "cultural_beliefs", "language")) ~ 1,
  ),
...
data_indicators <- data_indicators %>% 
  ...
  mutate(
    ...
    snfi_score = pmax(ifelse(rowMeans(select(.,!!!syms(snfi_non_critical_indicators)), na.rm = T)<0.99,
                                  ifelse(floor(((round(rowMeans(select(.,!!!syms(snfi_non_critical_indicators)), na.rm = T),digits = 2)-0.001)/0.3333)+1)>0,
                                         floor(((round(rowMeans(select(.,!!!syms(snfi_non_critical_indicators)), na.rm = T),digits = 2)-0.001)/0.3333)+1),1)
                                  ,
                                  3),na.rm = T),
    
    
    wash_score = pmax(!!!syms(wash_critical_indicators),
                      ifelse(rowMeans(select(.,!!!syms(wash_non_critical_indicators)), na.rm = T)<0.99,
                             ifelse(floor(((round(rowMeans(select(.,!!!syms(wash_non_critical_indicators)), na.rm = T),digits = 2)-0.001)/0.3333)+1)>0,
                                    floor(((round(rowMeans(select(.,!!!syms(wash_non_critical_indicators)), na.rm = T),digits = 2)-0.001)/0.3333)+1),1)
                             ,
                             3),na.rm = T),
    
    
    protection_score = pmax(!!!syms(protection_critical_indicators),
                            ifelse(rowMeans(select(.,!!!syms(protection_non_critical_indicators)), na.rm = T)<0.99,
                                   ifelse(floor(((round(rowMeans(select(.,!!!syms(protection_non_critical_indicators)), na.rm = T),digits = 2)-0.001)/0.3333)+1)>0,
                                          floor(((round(rowMeans(select(.,!!!syms(protection_non_critical_indicators)), na.rm = T),digits = 2)-0.001)/0.3333)+1),1)
                                   ,
                                   3),na.rm = T),
    
    fs_score = pmax(!!!syms(fs_critical_indicators),
                    ifelse(rowMeans(select(.,!!!syms(fs_non_critical_indicators)), na.rm = T)<0.99,
                           ifelse(floor(((round(rowMeans(select(.,!!!syms(fs_non_critical_indicators)), na.rm = T),digits = 2)-0.001)/0.3333)+1)>0,
                                  floor(((round(rowMeans(select(.,!!!syms(fs_non_critical_indicators)), na.rm = T),digits = 2)-0.001)/0.3333)+1),1)
                           ,
                           3),na.rm = T),
    
    
    health_score = pmax(!!!syms(health_critical_indicators),
                        ifelse(rowMeans(select(.,!!!syms(health_non_critical_indicators)), na.rm = T)<0.99,
                               ifelse(floor(((round(rowMeans(select(.,!!!syms(health_non_critical_indicators)), na.rm = T),digits = 2)-0.001)/0.3333)+1)>0,
                                      floor(((round(rowMeans(select(.,!!!syms(health_non_critical_indicators)), na.rm = T),digits = 2)-0.001)/0.3333)+1),1)
                               ,
                               3),na.rm = T),
    
    nutrition_score = pmax(!!!syms(nutrition_critical_indicators),
                    ifelse(rowMeans(select(.,!!!syms(nutrition_non_critical_indicators)), na.rm = T)<0.99,
                           ifelse(floor(((round(rowMeans(select(.,!!!syms(nutrition_non_critical_indicators)), na.rm = T),digits = 2)-0.001)/0.3333)+1)>0,
                                  floor(((round(rowMeans(select(.,!!!syms(nutrition_non_critical_indicators)), na.rm = T),digits = 2)-0.001)/0.3333)+1),1)
                           ,
                           3),na.rm = T),
    
    
    education_score = pmax(!!!syms(education_critical_indicators),
                           ifelse(rowMeans(select(.,!!!syms(education_non_critical_indicators)), na.rm = T)<0.99,
                                  ifelse(floor(((round(rowMeans(select(.,!!!syms(education_non_critical_indicators)), na.rm = T),digits = 2)-0.001)/0.3333)+1)>0,
                                         floor(((round(rowMeans(select(.,!!!syms(education_non_critical_indicators)), na.rm = T),digits = 2)-0.001)/0.3333)+1),1)
                                  ,
                                  3),na.rm = T),
  )
...
write.csv(data_indicators,'LSG_output_12_04.csv',row.names = F)
