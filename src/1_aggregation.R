...
if (!require("pacman")) install.packages("pacman") ## Loading pacman and packages
...
## Recoding colnames for select multiple questions 
...
data <- data %>%  new_recoding(target = nfi_access_dist_min_int_rec) %>%
  ...
  recode_to(to = "thirty", where = nfi_access_dist_min_int == 30 ) %>% 
  recode_to(to = "sixty", where = nfi_access_dist_min_int == 60 ) %>% 
  new_recoding(target = nfi_access_distance_max_rec) %>%
  recode_to(to = "less_than_30_minutes", where = nfi_access_distance_max <30  ) %>% 
  recode_to(to = "betwen_30_minutes_and_1_hour", where = nfi_access_distance_max >=30 & nfi_access_distance_max <60) %>% 
  recode_to(to = "betwen_1_and_2_hours", where = nfi_access_distance_max >=60 & nfi_access_distance_max <120) %>% 
  recode_to(to = "more_than_two_hours", where = nfi_access_distance_max >=120 ) %>% 
  end_recoding()  


# data$cccm_site_duration <- data$cccm_site_duration_rec
data$nfi_access_dist_min_int <- data$nfi_access_dist_min_int_rec
data$nfi_access_distance_max <- data$nfi_access_distance_max_rec


site_leadership <- c("site_manager", "comm_leader", "local_govt_rep", "ngo_rep", "gatekeeper") 

# data <- data %>% filter(ki_role %in% site_leadership) 

## Cleaning the env
rm(list = c("koboToolPath", 
            "listOfNumericQuestions",
...
select_multiple_questions <- import("input/aggregation/master_list.xlsx") %>% 
...
#select_multiple <- grep("[.]", names(data), value = TRUE)
...
## Output formatting 
select_multiple_output = data.frame(matrix(NA, ncol=1, nrow=nrow(aggregation_output_select_multiple))) 
select_multiple_output[,1] <- NULL

## Coding the binaries into xml labels and adding the question variable
sm_list <- gsub("\\..*","",colnames(aggregation_output_select_multiple)[str_detect(colnames(aggregation_output_select_multiple),"\\.")]) %>% unique() 

for (sm in sm_list) {
  
  sm_binary <-as.data.frame(aggregation_output_select_multiple %>% select(starts_with(sprintf("%s.",sm))))
  sm_formatted_and_binary <- cbind(select_multiple_output_formatting(sm,sm_binary),sm_binary)
  
  for (i in 2:ncol(sm_formatted_and_binary)) {
    sm_formatted_and_binary[,i] <- ifelse(sm_formatted_and_binary[,1]=="","",sm_formatted_and_binary[,i])
  }
  
  select_multiple_output <- cbind(select_multiple_output,sm_formatted_and_binary)
  
}


select_multiple_output <- cbind(aggregation_output_select_multiple[,1],select_multiple_output) 
